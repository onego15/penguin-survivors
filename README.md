...
**2キャラ・2ステージ・基本23武器＋進化8種・4匹のサポート**。キャラクターと背景はPrimitive Meshで構成し、BGM・効果音も同梱しています。外部アセット・プラグイン・Blenderは不要です。

![キャラとステージの選択画面](docs/screenshots/difficulty-title.png)

## ガイド

- [起動・操作](#起動操作)
- [4段階の難易度](docs/difficulty.md)
- [ポーズ・設定](#ポーズ設定)
- [ゲームの流れ](#ゲームの流れ)
- [サンドボックス](#サンドボックス)
- [リザルト：武器・必殺技・サポートの活躍](docs/results.md)
...
すべてのキャラ・ステージは最初から選択可能。同一起動中は選択を保持し、再挑戦は同じキャラ・ステージ・難易度で成長をリセットします。
...
| M／N | BGM／効果音のミュート |
| R | ゲームオーバー・クリア後に再挑戦 |
| Esc／パッドStart | ポーズ・設定。閉じると再開 |

タイトルではクリック／左右キーでキャラ、ステージボタンでステージを選びます。通常プレイにPythonは不要です。

## 難易度

タイトルで **やさしい／ノーマル／ハード／エキスパート** を選べます。初回はノーマル（従来のバランス）。本編中は変更できず、HUD・ポーズ・リザルトで確認できます。同一起動中のみ選択を保持します。

やさしいは敵の数・耐久・被ダメージを抑え、武器選択に必要なXPも軽減。ハード／エキスパートは敵の圧力を増やします。移動速度・弾速・予告時間は共通です。サンドボックスの「補助」にも独立した難易度選択を追加しています。

[倍率表・決戦増援・検証結果](docs/difficulty.md)
...
![ポーズ・音と見やすさの設定](docs/screenshots/pause-settings.png)
...
![サンドボックスの武器設定](docs/screenshots/evolution-sandbox.png)
![敵の選択とプレビュー](docs/screenshots/sandbox-enemies.png)

## ステージ別の武器

本編は共通9種＋限定7種の**各16種類**。きらきらプリズムは両ステージ共通です。雪原では常時周回する「くるくるパール」、城では長い待ち時間の後に巨大な星を落とす「おほしさまメテオ」を使えます。タイトルの一覧ボタンで所属を確認でき、サンドボックスでは基本23種類に加え、単体進化Lv.2〜5・合体Lv.1〜9も直接試せます。
...
2. **最大3候補から新武器・強化・進化を選択。** 選択中は戦闘停止。通常取得では装備が増え、進化・合体では素材を置き換えます。
...
**単体進化はLv.2から、合体は素材2武器がLv.1から。** 条件が揃うと、次のレベルアップで進化候補を1枠優先表示します。見送って通常の取得・強化を選ぶこともできます。

- ポップガンとハートは2方向へ分岐。現在のレベルを引き継ぎ、進化後もLv.5まで強化できます。
- 合体は元の2武器を1武器へ統合。`合体Lv＝素材Lv合計−1`、上限Lv.9。素材への強化投資も引き継ぎます。
- 進化・合体は合計2枠。消費した素材は、そのプレイでは再取得できません。
- 低レベル合体は、虹光線の幅・送風の到達と冷気・落雷の範囲を補強。威力とLv.9の最終性能は維持しています。
- タイトル・ポーズ設定の「進化一覧」でレシピを確認できます。[全8種の性能・成長・検証結果](docs/evolutions.md)も参照してください。

![単体進化の分岐選択](docs/screenshots/evolution-branches.png)

### リザルトで活躍を振り返る

クリア・死亡の両方で、武器別の実ダメージと撃破数、凍結・押し返し、必殺技とサポートの回復・軽減を確認できます。最多ダメージ・最多撃破を強調し、一覧は並べ替えとスクロールに対応。進化前の素材武器も別項目で残します。[集計ルールと画面例](docs/results.md)
...
| デン | お腹で着地し、近くの敵へ跳び、半径5mへ威力2＋4mの押し返し。最大5回 |
...
![デンの着地と押し返し](docs/screenshots/den-targeted-stomp.png)
...
デンは敵の集まりへ跳び、右上に押し返した数を表示。進化8武器の演出を更新し、ノクティスには安全な隙間を抜ける移動氷壁を追加しました。[仕様と検証結果](docs/den-noctis-update.md)

![ときめき虹プリズムの最終脈動](docs/screenshots/evolution-rich-prism-finale.png)
