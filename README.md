...
| 第二形態 | 氷アザラシ＋氷ユキヒョウ | 各1体ずつ。ユキヒョウは回り込み、HP14・速度3.2m/秒・接触10・XP3 | 演出終了6秒後、以後9秒ごと |

- 一度に最大2体、合計6体まで。プレイヤーから12〜16m離れた同じ120度の方向から登場します。上限中の出現は貯めません。
...
- 手下は初回6秒、第一形態12秒／第二形態9秒間隔、合計6体まで。第二形態はアザラシとユキヒョウを各1体。空き1枠なら少ない種類を補充し、同数なら交互に選びます。
...
`final_battle_test.gd` で6秒の初回、12秒／9秒周期、上限6体、混成と空き1枠の補充、サポート2回・退出後3秒・持ち越し・停止を確認。`ellipse_hud_test.gd` で楕円の四半周・半周・一周、移動後の手元への帰還、低FPS相当の往復命中判定、8m着弾、HUDの枠内配置を確認しました。既存の武器・援護・形態移行・勝利・再挑戦も回帰テストに成功しています。

90秒分の第二形態の時計を進める `boss_support_endurance_test.gd` では、サポートが8.1秒・41.8秒に登場し、手下は上限6体に収まりました。この試験は援護の接近・同行と時計を検証するため、敵の移動・攻撃を停止しています。
