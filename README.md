...
**2キャラ・2ステージ・基本23武器＋進化8種・4段階の難易度**。Primitive Meshで作ったキャラクターと、オリジナルのBGM・効果音を収録しています。
...
デンの跳躍援護、進化武器、両キャラの必殺技、雪原と氷の城、ノクティスの移動氷壁を紹介します。撮影用に装備・進行・配置を調整した実機映像です。**ボス第二形態とクリア画面のネタバレを含みます。** [収録内容・方法](docs/videos/README.md)
...
4. **決戦に挑む**：60秒ごとにWaveが変わり、中ボスを経て10分でラスボス戦へ。終了後はリザルトで武器と援護の活躍を確認できます。

ステージごとの基本武器プールは共通9種＋限定7種の16種類。**きらきらプリズムは両ステージ共通**です。強化は威力だけでなく、射程・範囲・弾数など武器に応じて成長します。[武器一覧](docs/weapons.md) · [進化レシピ](docs/evolutions.md)
...
タイトルから練習場へ入り、キャラ・基本23武器と進化8種・全敵・ボス両形態を自由に試せます。武器レベル、ランダムな敵の一括配置、無敵、敵の行動停止、必殺技の再充填に対応しています。
