...
[![Penguin Survivors デモ動画のプレビュー](docs/videos/demo-preview.gif)](docs/videos/penguin-survivors-demo.mp4)

**[音声付きデモ動画を見る（MP4・32秒・720p/60fps）](docs/videos/penguin-survivors-demo.mp4)**
...
![キャラとステージを選べるオープニング画面](docs/screenshots/castle-title.png)
...
| 武器 | 攻撃 | 初期ダメージ | 初期間隔 |
| --- | --- | --- | --- |
| ちゅるちゅるおうどん | 7m以内へ麺を伸ばし、周囲を巻き込んで戻る | 往路3＋復路3 | 2.4秒 |
| ぱたぱた扇風機 | 前方100度・4mの敵を2m押し返す | 1 | 3.2秒 |
...
![必殺技の満タン通知](docs/screenshots/ultimate-ready.png)
![王冠と吹雪が広がる必殺技](docs/screenshots/emperor-blizzard.png)
...
![メガネペンギンの選択](docs/screenshots/character-classic.png)
![ピンクペンギンの選択](docs/screenshots/character-pink.png)
![新武器・ハートの波動](docs/screenshots/heart-wave.png)
![ピンクペンギンの祝勝画面と19武器](docs/screenshots/pink-victory.png)
...
![ラブリー・ブルームとHP回復表示](docs/screenshots/lovely-bloom.png)
![技名を含む満タン通知](docs/screenshots/bloom-ready.png)
![必殺技の違いを示すキャラクター選択](docs/screenshots/bloom-title.png)
...
![城の門が対角に閉じた実画面](docs/screenshots/castle-closed-gates.png)
...
- 敵は通行グリッドで開いた門・壁の端へ迂回。突進は壁で停止します。
...
![城の通常敵4種と専用中ボス4体](docs/screenshots/castle-gallery.png)
...
![ノクティス第二形態の三つの氷輪予告](docs/screenshots/castle-noctis.png)

HP1600、接触18、速度2.0m毎秒。HP50%で第二形態へ移行し、速度2.4m毎秒、結晶の翼が展開します。登場・変身は2秒間の戦闘休止演出です。

1. **門の勅令**：対角2門を2秒予告後に閉鎖。第一形態6秒、第二形態8秒。占有中止も道中と共通。
2. **氷羽の扇**：1.2秒予告、固定方向へ5発／第二形態7発。速度6m毎秒、12ダメージ。
3. **月影の氷輪**：2秒予告、固定地点に24ダメージ。第一形態は半径3mの1地点、第二形態はプレイヤー付近と左右4.5mに半径2.5mの3地点。壁内や逃げ道のない配置は延期。

各技の後は1.5秒休止してから接近を再開。門の閉鎖処理中は他の技を重ねません。氷アザラシ／氷ユキヒョウの手下、第一形態1回・第二形態最大2回のサポートは雪原と共通です。手下を残していても王の撃破でクリアできます。
...
![城クリア・ピンクペンギンと19武器の結果表示](docs/screenshots/castle-clear.png)
...
| ちゅるちゅるおうどん | 威力3→9（往復各1回）、射程7→9.8m、巻き込み半径1.2→1.488m |

多発系はLv.2・4で威力、Lv.3・5で数を伸ばし、同じレベルで両方を急増させない設計です。最大強化武器は候補から外れます。全22武器MAX時だけ、レベルアップをHP20回復へ置き換えます。

![強化前後の数値を表示する武器選択](docs/screenshots/weapon-upgrade-values.png)

### ちゅるちゅるおうどん

両キャラで入手できる20番目の武器。どんぶりから麺を最寄りの敵へ伸ばし、先端付近を巻き込んで手元へ戻します。0.35秒で伸び、0.6秒で巻き戻り、同じ敵に往路・復路で各1回まで命中。最初につかんだ通常敵を最大2m引き寄せますが、プレイヤーから3m以内へ無理に引き込みません。ボスは引き寄せず、潜行中のモグラは狙いません。壁・閉じた門で麺が止まり、引き寄せも壁を越えません。

![どんぶりを持って麺で攻撃するピンクペンギン](docs/screenshots/udon-attack.png)

### 城のゴースト

Wave 6（5:00）から登場。紫の半透明の体、赤い目、尖った裾で識別します。閉じた**門だけ**をすり抜け、城壁は迂回。門を閉じる際の占有判定にも含めません。HP4・速度1.75m/秒・接触10を基礎とし、城の通常敵と同じ時間補正を適用。出現コスト2・経験値2、同時3体、特殊敵全体12体の上限を共有します。雪原には出現しません。

城は既存6種＋城固有5種の計11種類へ増えました。Wave 6で紹介し、Wave 9・10の混成にも登場します。

![閉じた門を通り抜けるゴーストのモデルと実画面](docs/screenshots/castle-ghost.png)
...
![20武器すべてが収まる祝勝画面](docs/screenshots/twenty-weapons-clear.png)
...
発動時の位置・向きを固定して、前方100度の敵へ1回攻撃。0.25秒かけて外向きに押し返します。敵がいなくても発動するため、発動に合わせて追手へ向き直ると退避路を作れます。

| 武器Lv. | 1 | 2 | 3 | 4 | 5 |
|---|---:|---:|---:|---:|---:|
| ダメージ | 1 | 2 | 2 | 3 | 3 |
| 間隔（秒） | 3.2 | 3.05 | 2.9 | 2.75 | 2.6 |
| 到達距離（m） | 4 | 4.3 | 4.6 | 4.9 | 5.2 |
| 押し返し（m） | 2 | 2 | 2.5 | 2.5 | 3 |

同じ敵への重ね掛けはなく、終了後0.6秒は再ノックバック不可。城壁・閉じた門・アリーナ端で止まり、追加の衝突ダメージはありません。ゴーストは門だけ通過できます。うどんの引き寄せよりノックバックを優先します。

![扇風機と前方の風・押し返しの残像](docs/screenshots/knockback-fan.png)
...
![薄い氷の結晶と雪の結晶アイコンで凍結を表示](docs/screenshots/ice-candy-freeze.png)
...
![制御武器の強化数値](docs/screenshots/control-upgrades.png)
![22武器の装備欄](docs/screenshots/twenty-two-weapons.png)
...
`tests/control_balance_audit.gd` は同じ5回分の選択（氷ブラスターLv.3＋羽根ショット／扇風機／アイスキャンディのいずれかLv.3）で比較。メガネ、seed73、道中7:00または決戦第二形態から最大35秒、追加選択・必殺技なし。自動回避は敵から離れる方向を優先し、扇風機に合わせて向き直る操作は行いません。表は「経過秒／終了HP／撃破数」です。
...
![22武器が収まる祝勝画面](docs/screenshots/twenty-two-clear.png)
