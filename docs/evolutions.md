...
基本26武器・各ステージ16種類の抽選に、進化武器11種類があります。進化は必須ではありません。プリズムも単体のまま強化できます。
...
| くるくるパール＋ひんやりチャイム | きらきらパールチャイム | 雪原・サンゴ浜 |
...
サンゴ浜でも共通素材による進化と、パール＋チャイムの合体を利用できます。ベルが出現しないため、雷雲ドームは城で作れます。海の3武器を使う合体を追加しました。詳細は以下を参照。


## サンゴ浜の合体進化

各素材Lv.1から候補になり、合体Lvは素材レベル合計−1（最大9）。既存進化と合わせて2枠を使用し、素材は再取得できません。基本武器のプールは各ステージ16種のままです。

| 素材 | 合体先 | 役割 |
|---|---|---|
| ざぶざぶ貝がら＋くるくるパール | ざぶざぶ真珠の大波 | 前方固定の広がる貫通波。合体後は周回しない |
| ぷかぷかバブル＋こなゆきドーム | ぷかぷか泡のアクアリウム | 固定ドームの周期攻撃と、外へ漂う泡の破裂 |
| ぱっちんカニばさみ＋ちゅるちゅるおうどん | ちゅるっとカニうどん | 左右から麺で引き寄せ、ハサミで挟む |

| 武器 | Lv.1→9の成長 |
|---|---|
| 真珠の大波 | 威力16→48、間隔2.8→2.2秒、射程12→16m、最大幅6→9m |
| アクアリウム | 周期威力2→6、間隔4.8→4秒、ドーム半径3→4m、泡威力4→12、破裂半径1.1→1.5m |
| カニうどん | 麺の威力2→5、間隔2.8→2.5秒、射程8→11m、ハサミ威力16→36、ハサミ半径1.5→2m |

均等補間し、威力のみ切り上げ。生成時の成長値を攻撃終了まで保持します。

- **大波**：速度7m/秒。発射時の位置・前方を固定し、幅30％から100％へ広がります。各敵に1回。真珠は波の一部で追加判定・防御・ノックバックなし。
- **アクアリウム**：16m以内の最寄りの敵の場所へ固定。0.5秒ごとに計6回攻撃し、同時に外周から60度ずつ方向を変えて泡を6個放出。泡は速度1.5m/秒・寿命3秒・接触半径0.45mで潮に流され、最初の接触で一度だけ破裂します。ドームは3秒で終了し、既に出た泡は寿命まで残ります。凍結・防御なし。
- **カニうどん**：0.35秒で左右へ伸び、0.9秒で巻き戻します。各側で最初の生存通常敵・手下1体を最大4m引き寄せ、発動地点から3m以内には引き込みません。終了時は捕縛対象の位置で挟み、不在なら左右3m。麺は往路1回、ハサミは左右合計1回まで。ボスはダメージのみ、ノックバックが優先。巻き戻し先は発動位置のままです。

地上判定は壁・閉門で遮断し、潜行中の敵は除外。泡は壁・寿命で消える場合に破裂しません。停止・死亡・構成変更は既存進化と共通。サンドボックスでは3種ともLv.1〜9で直接試せます。

![海の合体候補](screenshots/sea-fusion-choice.png)
![真珠の大波](screenshots/sea-fusion-pearl_wave.png)
![泡のアクアリウム](screenshots/sea-fusion-bubble_aquarium.png)
![カニうどん](screenshots/sea-fusion-crab_udon.png)
![オクトと3合体の表示](screenshots/sea-fusion-overlap.png)

### 海の合体の検証記録

Godot 4.7.2 / Compatibility / RTX 4060 Laptop。`beach_fusion`・`evolution`・`beach`・`beach_combat`・`sandbox`・`weapons`・`starfall_pool`・`settings`・`contributions` の9テストが通過。素材25組合せ、2枠、消去、単発判定、潮、捕縛、壁、潜行、ボス、停止、報酬を確認しました。既存の証明書ストア・終了時リソース解放警告は残っています。

両キャラ×seed17/73/211×Lv.1/5/9×3武器×密集/散開/オクト×素材/合体の324条件を各12秒比較。左右へ短く移動し、HPを毎フレーム100へ戻して同じ時間だけ戦わせる自動操作です。被ダメージは無敵時間を含む通常処理で集計。オクトのHPのみ測定用に100000へ増やし、撃破時間ではなく与ダメージを測っています。人間の生存率・クリア難易度の保証ではありません。

初期武器Lv.1を共通装備にして、合体Lv.Lに必要なL＋2回の選択を、比較側では素材へ均等配分します。Lv.9比較は素材が両方Lv.5で上限になるため、余る1回を初期武器の強化へ使います。

以下はLv.1の両キャラ・3シード平均。「素材強化 → 合体」の順です。Lv.5/9を含む全行は[生データ](benchmarks/beach-fusion-balance.json)に記録しています。

| 合体 | 配置 | 撃破数 | 被ダメージ | 与ダメージ |
|---|---|---:|---:|---:|
| 真珠の大波 | 密集 | 8.2 → 5.2 | 3.3 → 3.3 | 96.5 → 71.3 |
| 真珠の大波 | 散開 | 3.2 → 5.2 | 20.0 → 16.7 | 46.2 → 64.8 |
| 真珠の大波 | オクト | 0.0 → 0.0 | 0.0 → 0.0 | 60.5 → 56.5 |
| アクアリウム | 密集 | 8.2 → 15.7 | 3.3 → 0.0 | 111.7 → 153.2 |
| アクアリウム | 散開 | 4.8 → 6.2 | 20.0 → 20.0 | 62.0 → 78.3 |
| アクアリウム | オクト | 0.0 → 0.0 | 0.0 → 0.0 | 68.5 → 76.5 |
| カニうどん | 密集 | 4.2 → 3.7 | 3.3 → 3.3 | 65.2 → 57.5 |
| カニうどん | 散開 | 4.0 → 3.7 | 20.0 → 20.0 | 48.3 → 45.5 |
| カニうどん | オクト | 0.0 → 0.0 | 0.0 → 0.0 | 86.5 → 60.5 |

大波は散開、アクアリウムは密集で強みが出ました。カニうどんは低ランクの不足を補うため、初案からLv.1ハサミ威力12→16・間隔3.2→2.8秒へ調整（Lv.9据え置き）。再比較でもこの自動経路では素材強化を下回る場面が残り、自動照準の代わりとして無条件に有利にはなっていません。敵を左右に集める位置取りを要する武器です。

描画負荷はseed73・海の敵96体・静止無敵・4武器Lv.3・60フレーム準備後360フレームを計測。海の基本3武器＋ポップガンは平均2.22ms／p95 4.97ms、海の合体3武器＋ポップガンは平均2.83ms／p95 5.95ms。合体3種同時は表示負荷確認用で、本編の2枠を超える構成です。異なる攻撃構成を各1回測った結果で、性能保証ではありません。[描画記録](benchmarks/beach-fusion-performance.json)。

候補カード、個別攻撃、敵予告・オクトとの重なり、37武器のモデル一覧を実画面で確認。デモ動画は更新していません。
