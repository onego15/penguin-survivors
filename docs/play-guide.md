...
タイトルの「サンドボックス」から、自由に戦闘を試せる練習場へ入れます。**Tab／設定ボタン**で一時停止し、2キャラ・基本26武器と進化11種・敵編成を変更できます。
...
最後の30秒は増援が止まり、城門も全開になります。通常敵は倒し、中ボスはHPを削るほど、ラスボスへのHP加算が減ります。全滅しても決戦は10分開始です。

- 通常敵：掃討開始時の数に対する残存割合 × 30％。潜行中のモグラも含みます。
- 中ボス：残りHP ÷ 難易度補正後の最大HP × 20％。掃討開始前に与えたダメージも有効です。
- 2つを足してから％単位で切り上げ、最大50％。通常敵40体中10体・中ボスHP25％なら、7.5＋5＝12.5 → **ボスHP＋13％**。
- 加算は難易度補正後のボスHPへ一度だけ適用。攻撃力・速度・技は変わりません。全滅なら加算なしです。

HUDに残り時間・残敵数・中ボスHP・加算見込みと内訳を表示。画面外の残敵へ白い矢印で案内します。撃破報酬と武器選択は通常どおりで、ポーズ・武器選択中は掃討時計も停止します。10分で残敵を報酬なしで整理し、加算率を確定します。サンドボックスには自動適用しません。

![掃討の残敵数とHP加算見込み](screenshots/cleanup-hud.png)

## サンゴ浜の状態異常

砂で移動が遅くなり、しびれで次の自動攻撃が遅れ、墨で画面外周が見えにくくなります。いずれも短時間で自然解除。緑の泉へ入るとすぐ解除できますが、HPは回復しません。[潮・泉・各状態の詳細](beach.md)
