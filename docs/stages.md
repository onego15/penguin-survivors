...
| 3 | キツネ「狐姫・アンバー」 | オコジョ「白影・シルク」：進路を横へ避ける低空飛び込み |
...
2. **氷棘の城壁**：門を全開にして2.4秒予告。城壁からトゲ付きの攻撃用氷壁が分離して動きます。第一形態は1列・3m/秒・24ダメージ、第二形態は左右2列・3.5m/秒・28ダメージ。共通の幅4mの隙間と南北の外周が退避先。各列は1回まで命中し、他の攻撃・新規増援を重ねず、終了後2秒休止します。
3. **氷羽の扇**：1.2秒予告した方向へ5発／第二形態7発。速度6m/秒・12ダメージ。閉門を貫通します。
4. **月影の氷輪**：2秒予告・24ダメージ。第一形態は半径3mの1地点、第二形態は半径2.5mの3地点。予告後は追従せず、逃げ道がない配置は延期します。
...
手下を倒し切る必要はなく、ボス撃破でクリア。ペンギンとサポート4匹が祝福し、時間・撃破数・レベル・取得武器を表示します。プレイヤーが同時に倒れた場合は死亡を優先します。

## 潮風のサンゴ浜

3番目のステージは最初から選択できます。新敵6種・専用中ボス4体・潮を操るオクト、状態異常と泉、専用武器3種の仕様は[サンゴ浜ガイド](beach.md)へ。9:30の掃討と残敵HP加算は共通です。

## ステージ別武器プール

本編は共通9種＋ステージ別7種の各16種類から、新規取得と強化を混ぜて抽選します。**きらきらプリズムは全ステージ共通**。キャラの初期装備も共通枠です。進化を含む取得・強化候補がなくなればレベルアップでHP20回復。サンドボックスでは所属に関係なく全26種類を試せます。

共通：きらきらプリズム、こおりのポップガン、ときめきハート、ふわふわ羽根ショット、ひんやりチャイム、おかえりおさかな、ちゅるちゅるおうどん、ぱたぱた扇風機、ひえひえアイスキャンディ。

| 雪原 | 夜の氷の城 | 潮風のサンゴ浜 |
|---|---|---|
| すいすいつらら | ぴかぴかベル | くるくるパール |
| ぽかぽかおひさま | こなゆきドーム | こなゆきドーム |
| くるくるパール | ゆらゆらリボン | ぱちぱち花火 |
| ころころどんぐり | ゆきだるまシューター | おほしさまメテオ |
| あつあつスケート | ぶんぶんロケット | ざぶざぶ貝がら |
| ころりんアイスボール | しっぽのクラッカー | ぷかぷかバブル |
| ぱちぱち花火 | おほしさまメテオ | ぱっちんカニばさみ |

雪原には羽根・つららの前方攻撃と後方の花火、城にはリボンの前方攻撃と後方散弾があります。引き寄せ・風・凍結は全ステージで利用可能です。タイトルの「出現武器16種を見る」で選択中の一覧を確認できます。

![タイトルのステージ別一覧](screenshots/stage-weapon-list.png)

氷棘の城壁は地形を塞がず、壁・門を通過するボス攻撃です。難易度・シロクマ軽減・無敵を適用し、通常の敵弾消去では消えません。既存技の待ち時間は3秒。予告開始時点で歩いて退避できる配置を検証し、不成立なら再試行します。

![移動する氷壁と安全な隙間](screenshots/noctis-moving-walls.png)

## 敵の見た目・行動の更新（2026-09-20）

通常カメは基礎HP6、時間HP倍率の0.75乗。生成4秒後から8秒周期で、準備0.4秒→半減防御1.2秒→解除0.4秒。防御の一連の動作中は移動・接触を止め、凍結・ノックバックで中断します。モスバックのHP・防御性能は従来どおりです。

オコジョは前方1m＋横1.5mへ斜めに跳び、着地後0.35秒休止。ヤギは4.5m以内で予告し、4mの頭突き後1.2秒、壁衝突時1.8秒休止します。シルクは高さ0.45mの低空飛び込みで進路へ20ダメージ1回。着地爆発はなく、ルナの高い跳躍と避け方が異なります。

突進予告は壁・外周までの実際の移動に合わせ、アンバー／ヴェスパーは扇状、ハリネズミは8方向の射線を表示します。

[変更内容・比較検証・全敵モデル](enemy-refresh.md)

![通常敵15種](screenshots/enemy-refresh-normal.png)

## 決戦への移行

両ステージとも9:30で新規の通常敵・中ボス出現を停止し、30秒の掃討へ移行します。城門は全開固定。10分時点の通常敵の残存割合（最大30％）と中ボスの残りHP割合（最大20％）をラスボスHPへ加算します。[計算と表示](play-guide.md)を参照。
