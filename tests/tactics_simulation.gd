	...
	for start_time in [190,310,430]:
		for build in builds:
			...
			while game.elapsed<start_time+22 and not game.game_over:
				...
				await process_frame
			print("TACTICS wave=%d build=%s time=%.1f hp=%d kills=%d special=%d shots=%d clouds=%d" % [start_time/60+1,build,game.elapsed-start_time,game.player.health,game.kills,max_special,max_shots,max_clouds])
