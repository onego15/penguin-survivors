...
func run() -> void:
	...
	game._update_hud()
	var cold:=OS.get_cmdline_user_args().has("--cold")
	...
	for i in range(40):
		var effect=load("res://scripts/blizzard_visual.gd" if cold else "res://scripts/weapon_detail.gd" if revised and i%6!=3 else "res://scripts/weapon_flourish.gd").new()
	...
	root.get_texture().get_image().save_png("res://docs/screenshots/cold-stress.png" if cold else "res://docs/screenshots/weapon-stress-revised.png" if revised else "res://docs/screenshots/weapon-stress.png")
