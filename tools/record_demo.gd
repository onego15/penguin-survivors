...
func start() -> void:
	...
	caption=Label.new(); caption.position=Vector2(360,647)
	...
	caption.text="雪原：くるくるパールと、新しい仲間"
...
func chapter(stage_id: String, character_id: String) -> void:
	...
	for i in range(24):
		...
		if enemy!=null: enemy.position=Vector3(cos(i*TAU/24),0,sin(i*TAU/24))*(9+i%4)
	game.camera.size=20
func tick() -> void:
	frame+=1
	...
	if frame==2*FPS:
		...
		caption.text="デン：お腹でどすん！ 周囲の敵を押し返す"
	...
	if frame==24*FPS: game.ultimate.reward(200)
	if frame==26*FPS:
	...
	if frame==42*FPS and is_instance_valid(game.active_boss):
		game.active_boss.take_damage(maxi(1,game.active_boss.health-790))
		caption.text="第二形態：変わる城門と、月影の氷輪"
	if frame==54*FPS and is_instance_valid(game.active_boss):
